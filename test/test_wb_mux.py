...
class TestWbMux(TestCase):
    ...
    def setUp(self):
        ...
        self.result_in, self.rdata_in, self.wb_out, self.wb_out_v = [
            Signal(intbv(0, min=sf['SIGNED_MIN_VALUE'], max=sf['SIGNED_MAX_VALUE'])) for i in range(4)
        ]
        self.dut = mux32bit2to1(self.mem_to_reg, self.result_in, self.rdata_in, self.wb_out)
    ...
    def testHoldDeassertVerilog(self):
        ...
        dut_v = mux32bit2to1_v(self.mem_to_reg, self.result_in, self.rdata_in, self.wb_out_v)
